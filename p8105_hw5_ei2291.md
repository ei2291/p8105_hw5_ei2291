...
Run `prop.test` for baltimore and get the estimate with CI
...
Based on the output, about 64.6% with a 95% CI (0.63 to 0.66) of
homicides in baltimore are unsolved.

Run a `prop.test` for every city

``` r
city_results=
  homicide_summary |> 
  mutate(
    test= map2(unsolved, total, \(u, n) prop.test(u,n)),
...
```
    ...
    ## ℹ In argument: `test = map2(unsolved, total, function(u, n) prop.test(u, n))`.
...
city_results_small |> 
  ...
  labs(
    title = "Unsolved homicide proportion with 95% CI by city",
...
<img src="p8105_hw5_ei2291_files/figure-gfm/unnamed-chunk-21-1.png" width="90%" />

Based on the plot we see that there is a range in the proportion of
unsolved homicides. Some city have low proportion(e.g.Richmond,VA),
while others have higher unsolved proportions(e.g.Chicago, IL).Overall
though many cities have around 40 to 60% of unsolved homicide
proportions.
